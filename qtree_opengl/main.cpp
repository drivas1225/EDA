...
//Crear quad tree
    ...
    QuadTree qtree = QuadTree(boundary,5000 );
    Rectan range;
    vector<Point> points;
    double prom_x = 0;
    double prom_y = 0;
...
vector<Point> csvLector(string g)
{
    ...
    while(getline(archivodata,line))
    {
            ...
            cont++;
            double x= atof(longitud.c_str());
            double y= atof(latitud.c_str());
            prom_x=prom_x+x;
            prom_y=prom_y+y;
            Point a(x,y);
...
int main(int argc, char** argv) {
    ...
    */
     ...
     archivo2 = "C:/Users/Lenovo/Desktop/Quadtree_gl/qtree_opengl/country-capitals.csv";
     int parada,ar;

    //cout<<"archivo"<<p_archivos.size();
    cout<<"ingresar criterio de parada: ";
    cin >> parada;
    qtree.capacity = parada;
    cout<<"archivos1: crime"<<endl;
    cout<<"archivos2: capitals"<<endl;
     cin >> ar;
     if (ar == 1){
     ...
     if (ar == 2){
        vector <Point> p_archivos = csvLector(archivo2);
        for(int i = 0 ; i < p_archivos.size();i++){
        //cout<<p_archivos[i].x <<" "<<p_archivos[i].y<<endl;
        qtree.insert_p(p_archivos[i]);
    }
